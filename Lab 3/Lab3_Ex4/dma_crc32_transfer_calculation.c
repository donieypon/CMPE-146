...
// used to keep track of remaining data size with DMA CRC
int size = 0;
...
int main(void)
{
    ...
    for (jj = 0; jj < 8; jj++) {
        ...
        // if data block size > 1024, main sets up first transfer for 1024 bytes
        if (size >= 1024) {
...
{
    // update size after handling
    size -= 1024;

    MAP_DMA_setChannelControl(UDMA_PRI_SELECT,
                              UDMA_SIZE_8 | UDMA_SRC_INC_8 | UDMA_DST_INC_NONE | UDMA_ARB_1024);

    // if size is still > 1024, initiate transfers inside DMA
    while (size != 0) {
        if (size >= 1024) {
            MAP_DMA_setChannelTransfer(UDMA_PRI_SELECT,
                                       UDMA_MODE_AUTO, data_array,
